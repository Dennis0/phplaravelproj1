...
class DbHandler
{
    ...
    private $conn;
    require_once '../include/PassHash2.php';
...
closed,maintenance,vatinclusive,totalCharges,invoiceFinancier,transferjobcard,contactPersonCommission)
                    VALUES(NOW(),Now(),?,?,?,?,?,?,?,?,?
                      ,0,0,0,0,0,0,0,0,?);");

    if ($stmt->bind_param("ssssssssss" ,$user, $sname, $phone, $chasis, $engine,
        $color, $location, $charges,$model,$inst)
    ) {
        if ($stmt->execute()) {
